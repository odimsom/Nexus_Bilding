    ...
    : IRequestHandler<UpsertNoSeriesCommand, bool>
{
    public async Task<bool> Handle(UpsertNoSeriesCommand cmd, CancellationToken ct)
    {
        ...
        {
            var result = NexusBilling.Core.Domain.Administration.Entities.NoSeries.Create(TenantIdentifier.Create(cmd.TenantId), cmd.Code, cmd.Description);
    ...
    : IRequestHandler<UpsertNoSeriesLineCommand, bool>
{
    public async Task<bool> Handle(UpsertNoSeriesLineCommand cmd, CancellationToken ct)
    {
        ...
        {
            var result = NexusBilling.Core.Domain.Administration.Entities.NoSeriesLine.Create(tid, cmd.SeriesCode, cmd.StartingNo, cmd.EndingNo);
            if (!result.IsSuccess) return false;
