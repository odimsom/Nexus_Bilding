    ...
    : IRequestHandler<CreatePurchaseOrderCommand, CreatePurchaseOrderResult>
{
    ...
    public async Task<CreatePurchaseOrderResult> Handle(CreatePurchaseOrderCommand cmd, CancellationToken ct)
    {
        ...
        var headerResult = PurchaseHeader.Create(
            tid, "Order", docNo,
            cmd.BuyFromVendorNo, cmd.PayToName,
            Utc(cmd.PostingDate));
        ...
        foreach (var l in cmd.Lines)
        {
            var lineResult = PurchaseLine.Create(
                ...
                buyFromVendorNo: cmd.BuyFromVendorNo,
                documentNo: docNo,
                lineNo: lineNo,
                ...
                lineDiscountAmount: 0m, // Compute below
                amount: 0m,
                ...
                qtyRcdNotInvoiced: 0m,
                amtRcdNotInvoiced: 0m,
                quantityReceived: 0m,
                quantityInvoiced: 0m,
                ...
                currencyCode: null,
                outstandingAmountLcy: 0m,
                amtRcdNotInvoicedLcy: 0m,
                blanketOrderNo: string.Empty,
                blanketOrderLineNo: 0,
                vatBaseAmount: 0m,
                unitCost: 0m,
                systemCreatedEntry: false,
                lineAmount: 0m,
                vatDifference: 0m,
                invDiscAmountToInvoice: 0m,
                ...
                prepaymentTaxLiable: false,
                prepaymentTaxGroupCode: string.Empty,
                prepmtAmtToDeduct: 0m,
                prepmtAmtDeducted: 0m,
                prepaymentLine: false,
                prepmtAmountInvInclVat: 0m,
                prepmtAmountInvLcy: 0m,
                icPartnerCode: string.Empty,
                prepmtVatAmountInvLcy: 0m,
                prepaymentVatDifference: 0m,
                prepmtVatDiffToDeduct: 0m,
                prepmtVatDiffDeducted: 0m,
                outstandingAmtExVatLcy: 0m,
                aRcdNotInvExVatLcy: 0m,
                dimensionSetId: 0,
                ...
                jobTotalPrice: 0m,
                jobLineAmount: 0m,
                jobLineDiscountAmount: 0m,
                jobLineDiscount: 0m,
                jobUnitPriceLcy: 0m,
                jobTotalPriceLcy: 0m,
                jobLineAmountLcy: 0m,
                jobLineDiscAmountLcy: 0m,
                jobCurrencyFactor: 0m,
                jobCurrencyCode: string.Empty,
                jobPlanningLineNo: 0,
                jobRemainingQty: 0m,
                jobRemainingQtyBase: 0m,
                deferralCode: string.Empty,
                returnsDeferralStartDate: null,
                ...
                quantityBase: 0m,
                outstandingQtyBase: 0m,
                qtyToInvoiceBase: 0m,
                qtyToReceiveBase: 0m,
                qtyRcdNotInvoicedBase: 0m,
                qtyReceivedBase: 0m,
                qtyInvoicedBase: 0m,
                faPostingDate: null,
                faPostingType: 0,
                depreciationBookCode: string.Empty,
                salvageValue: 0m,
                deprUntilFaPostingDate: false,
                deprAcquisitionCost: false,
                maintenanceCode: null,
                insuranceNo: null,
                budgetedFaNo: string.Empty,
                duplicateInDepreciationBook: string.Empty,
                useDuplicationList: false,
                responsibilityCenter: string.Empty,
                crossReferenceNo: string.Empty,
                unitOfMeasureCrossRef: string.Empty,
                crossReferenceType: 0,
                crossReferenceTypeNo: string.Empty,
                itemCategoryCode: string.Empty,
                nonstock: false,
                purchasingCode: null,
                productGroupCode: string.Empty,
                specialOrder: false,
                specialOrderSalesNo: string.Empty,
                specialOrderSalesLineNo: 0,
                completelyReceived: false,
                requestedReceiptDate: null,
                promisedReceiptDate: null,
                leadTimeCalculation: string.Empty,
                inboundWhseHandlingTime: string.Empty,
                plannedReceiptDate: null,
                orderDate: null,
                allowItemChargeAssignment: false,
                returnQtyToShip: 0m,
                returnQtyToShipBase: 0m,
                returnQtyShippedNotInvd: 0m,
                retQtyShpdNotInvdBase: 0m,
                returnShpdNotInvd: 0m,
                returnShpdNotInvdLcy: 0m,
                returnQtyShipped: 0m,
                returnQtyShippedBase: 0m,
                returnShipmentNo: string.Empty,
                returnShipmentLineNo: 0,
            ...
            // Using reflection because these properties are private set and Create has them as parameters
            var amountProp = typeof(PurchaseLine).GetProperty("Amount");
            amountProp?.SetValue(line, Math.Round(lineAmt, 2));

            var amountIncVatProp = typeof(PurchaseLine).GetProperty("AmountIncludingVat");
            amountIncVatProp?.SetValue(line, Math.Round(lineAmt * (1 + VatRate), 2));

            var outAmtProp = typeof(PurchaseLine).GetProperty("OutstandingAmount");
            outAmtProp?.SetValue(line, Math.Round(lineAmt, 2));
        ...
        return new CreatePurchaseOrderResult(docNo);
    }
}
