...
public class FiledContractLine : Entity
{
    private FiledContractLine() { }
    ...
    public static OperationResult<FiledContractLine, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<FiledContractLine, DomainError>.Fail(DomainError.Validation("administration.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new FiledContractLine()
        {
            TenantId = tenantId
        };
        return OperationResult<FiledContractLine, DomainError>.Ok(entity);
    }
}
