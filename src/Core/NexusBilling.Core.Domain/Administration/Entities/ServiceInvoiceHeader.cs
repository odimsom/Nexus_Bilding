...
public class ServiceInvoiceHeader : Entity
{
    private ServiceInvoiceHeader() { }
    ...
    public static OperationResult<ServiceInvoiceHeader, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<ServiceInvoiceHeader, DomainError>.Fail(DomainError.Validation("administration.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new ServiceInvoiceHeader()
        {
            TenantId = tenantId
        };
        return OperationResult<ServiceInvoiceHeader, DomainError>.Ok(entity);
    }
}
