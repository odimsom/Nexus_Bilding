...
public class ServiceInvoiceLine : Entity
{
    private ServiceInvoiceLine() { }
    ...
    public bool SubstitutionAvailable { get; private set; }
    ...
    public static OperationResult<ServiceInvoiceLine, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<ServiceInvoiceLine, DomainError>.Fail(DomainError.Validation("administration.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new ServiceInvoiceLine()
        {
            TenantId = tenantId
        };
        return OperationResult<ServiceInvoiceLine, DomainError>.Ok(entity);
    }
}
