...
public class ServiceShipmentHeader : Entity
{
    private ServiceShipmentHeader() { }
    ...
    public static OperationResult<ServiceShipmentHeader, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<ServiceShipmentHeader, DomainError>.Fail(DomainError.Validation("administration.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new ServiceShipmentHeader()
        {
            TenantId = tenantId
        };
        return OperationResult<ServiceShipmentHeader, DomainError>.Ok(entity);
    }
}
