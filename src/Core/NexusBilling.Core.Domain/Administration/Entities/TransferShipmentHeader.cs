...
public class TransferShipmentHeader : Entity
{
    private TransferShipmentHeader() { }
    ...
    public static OperationResult<TransferShipmentHeader, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<TransferShipmentHeader, DomainError>.Fail(DomainError.Validation("administration.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new TransferShipmentHeader()
        {
            TenantId = tenantId
        };
        return OperationResult<TransferShipmentHeader, DomainError>.Ok(entity);
    }
}
