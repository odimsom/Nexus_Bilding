...
public class PostedInvtPickLine : Entity
{
    private PostedInvtPickLine() { }
    ...
    public string WhseActivityNo { get; private set; }
    ...
    public static OperationResult<PostedInvtPickLine, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<PostedInvtPickLine, DomainError>.Fail(DomainError.Validation("inventory.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new PostedInvtPickLine()
        {
            TenantId = tenantId
        };
        return OperationResult<PostedInvtPickLine, DomainError>.Ok(entity);
    }
}
