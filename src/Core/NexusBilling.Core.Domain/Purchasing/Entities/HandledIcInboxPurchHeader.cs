...
public class HandledIcInboxPurchHeader : Entity
{
    private HandledIcInboxPurchHeader() { }
    ...
    public static OperationResult<HandledIcInboxPurchHeader, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<HandledIcInboxPurchHeader, DomainError>.Fail(DomainError.Validation("purchasing.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new HandledIcInboxPurchHeader()
        {
            TenantId = tenantId
        };
        return OperationResult<HandledIcInboxPurchHeader, DomainError>.Ok(entity);
    }
}
