...
public class HandledIcInboxSalesLine : Entity
{
    private HandledIcInboxSalesLine() { }
    ...
    public static OperationResult<HandledIcInboxSalesLine, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<HandledIcInboxSalesLine, DomainError>.Fail(DomainError.Validation("purchasing.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new HandledIcInboxSalesLine()
        {
            TenantId = tenantId
        };
        return OperationResult<HandledIcInboxSalesLine, DomainError>.Ok(entity);
    }
}
