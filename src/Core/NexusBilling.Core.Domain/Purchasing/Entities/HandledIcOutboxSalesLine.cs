...
public class HandledIcOutboxSalesLine : Entity
{
    private HandledIcOutboxSalesLine() { }
    ...
    public static OperationResult<HandledIcOutboxSalesLine, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<HandledIcOutboxSalesLine, DomainError>.Fail(DomainError.Validation("purchasing.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new HandledIcOutboxSalesLine()
        {
            TenantId = tenantId
        };
        return OperationResult<HandledIcOutboxSalesLine, DomainError>.Ok(entity);
    }
}
