...
public class IcInboxPurchaseHeader : Entity
{
    private IcInboxPurchaseHeader() { }
    ...
    public static OperationResult<IcInboxPurchaseHeader, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<IcInboxPurchaseHeader, DomainError>.Fail(DomainError.Validation("purchasing.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new IcInboxPurchaseHeader()
        {
            TenantId = tenantId
        };
        return OperationResult<IcInboxPurchaseHeader, DomainError>.Ok(entity);
    }
}
