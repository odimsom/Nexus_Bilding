...
public class IcInboxPurchaseLine : Entity
{
    private IcInboxPurchaseLine() { }
    ...
    public static OperationResult<IcInboxPurchaseLine, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<IcInboxPurchaseLine, DomainError>.Fail(DomainError.Validation("purchasing.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new IcInboxPurchaseLine()
        {
            TenantId = tenantId
        };
        return OperationResult<IcInboxPurchaseLine, DomainError>.Ok(entity);
    }
}
