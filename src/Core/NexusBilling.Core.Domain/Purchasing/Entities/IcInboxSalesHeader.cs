...
public class IcInboxSalesHeader : Entity
{
    private IcInboxSalesHeader() { }
    ...
    public static OperationResult<IcInboxSalesHeader, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<IcInboxSalesHeader, DomainError>.Fail(DomainError.Validation("purchasing.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new IcInboxSalesHeader()
        {
            TenantId = tenantId
        };
        return OperationResult<IcInboxSalesHeader, DomainError>.Ok(entity);
    }
}
