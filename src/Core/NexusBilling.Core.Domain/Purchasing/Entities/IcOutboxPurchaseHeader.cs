...
public class IcOutboxPurchaseHeader : Entity
{
    private IcOutboxPurchaseHeader() { }
    ...
    public static OperationResult<IcOutboxPurchaseHeader, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<IcOutboxPurchaseHeader, DomainError>.Fail(DomainError.Validation("purchasing.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new IcOutboxPurchaseHeader()
        {
            TenantId = tenantId
        };
        return OperationResult<IcOutboxPurchaseHeader, DomainError>.Ok(entity);
    }
}
