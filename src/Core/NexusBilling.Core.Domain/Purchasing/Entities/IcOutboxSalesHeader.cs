...
public class IcOutboxSalesHeader : Entity
{
    private IcOutboxSalesHeader() { }
    ...
    public static OperationResult<IcOutboxSalesHeader, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<IcOutboxSalesHeader, DomainError>.Fail(DomainError.Validation("purchasing.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new IcOutboxSalesHeader()
        {
            TenantId = tenantId
        };
        return OperationResult<IcOutboxSalesHeader, DomainError>.Ok(entity);
    }
}
