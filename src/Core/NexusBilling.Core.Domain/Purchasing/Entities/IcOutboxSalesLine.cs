...
public class IcOutboxSalesLine : Entity
{
    private IcOutboxSalesLine() { }
    ...
    public static OperationResult<IcOutboxSalesLine, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<IcOutboxSalesLine, DomainError>.Fail(DomainError.Validation("purchasing.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new IcOutboxSalesLine()
        {
            TenantId = tenantId
        };
        return OperationResult<IcOutboxSalesLine, DomainError>.Ok(entity);
    }
}
