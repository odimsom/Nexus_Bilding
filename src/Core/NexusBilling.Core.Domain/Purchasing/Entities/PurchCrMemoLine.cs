...
public class PurchCrMemoLine : Entity
{
    private PurchCrMemoLine() { }
    ...
    public static OperationResult<PurchCrMemoLine, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<PurchCrMemoLine, DomainError>.Fail(DomainError.Validation("purchasing.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new PurchCrMemoLine()
        {
            TenantId = tenantId
        };
        return OperationResult<PurchCrMemoLine, DomainError>.Ok(entity);
    }
}
