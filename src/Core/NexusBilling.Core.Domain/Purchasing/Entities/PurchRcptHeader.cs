...
public class PurchRcptHeader : Entity
{
    private PurchRcptHeader() { }
    ...
    public static OperationResult<PurchRcptHeader, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<PurchRcptHeader, DomainError>.Fail(DomainError.Validation("purchasing.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new PurchRcptHeader()
        {
            TenantId = tenantId
        };
        return OperationResult<PurchRcptHeader, DomainError>.Ok(entity);
    }
}
