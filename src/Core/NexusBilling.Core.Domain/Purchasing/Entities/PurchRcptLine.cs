...
public class PurchRcptLine : Entity
{
    private PurchRcptLine() { }
    ...
    public static OperationResult<PurchRcptLine, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<PurchRcptLine, DomainError>.Fail(DomainError.Validation("purchasing.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new PurchRcptLine()
        {
            TenantId = tenantId
        };
        return OperationResult<PurchRcptLine, DomainError>.Ok(entity);
    }
}
