...
public class ReturnShipmentHeader : Entity
{
    private ReturnShipmentHeader() { }
    ...
    public static OperationResult<ReturnShipmentHeader, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<ReturnShipmentHeader, DomainError>.Fail(DomainError.Validation("purchasing.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new ReturnShipmentHeader()
        {
            TenantId = tenantId
        };
        return OperationResult<ReturnShipmentHeader, DomainError>.Ok(entity);
    }
}
