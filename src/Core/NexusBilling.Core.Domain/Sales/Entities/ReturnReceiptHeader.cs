...
public class ReturnReceiptHeader : Entity
{
    private ReturnReceiptHeader() { }
    ...
    public string WarehouseHandlingTime { get; private set; }
    public bool LateOrderShipping { get; private set; }
    ...
    public static OperationResult<ReturnReceiptHeader, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<ReturnReceiptHeader, DomainError>.Fail(DomainError.Validation("sales.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new ReturnReceiptHeader()
        {
            TenantId = tenantId
        };
        return OperationResult<ReturnReceiptHeader, DomainError>.Ok(entity);
    }
}
