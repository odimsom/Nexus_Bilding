...
public class ReturnReceiptLine : Entity
{
    private ReturnReceiptLine() { }
    ...
    public static OperationResult<ReturnReceiptLine, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<ReturnReceiptLine, DomainError>.Fail(DomainError.Validation("sales.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new ReturnReceiptLine()
        {
            TenantId = tenantId
        };
        return OperationResult<ReturnReceiptLine, DomainError>.Ok(entity);
    }
}
