...
public class SalesCrMemoLine : Entity
{
    private SalesCrMemoLine() { }
    ...
    public static OperationResult<SalesCrMemoLine, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<SalesCrMemoLine, DomainError>.Fail(DomainError.Validation("sales.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new SalesCrMemoLine()
        {
            TenantId = tenantId
        };
        return OperationResult<SalesCrMemoLine, DomainError>.Ok(entity);
    }
}
