...
public class PostedApprovalEntry : Entity
{
    private PostedApprovalEntry() { }
    ...
    public int NumberOfApprovedRequests { get; private set; }
    public int NumberOfRejectedRequests { get; private set; }
    public int IterationNo { get; private set; }
    public int EntryNo { get; private set; }

    public static OperationResult<PostedApprovalEntry, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<PostedApprovalEntry, DomainError>.Fail(DomainError.Validation("security.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new PostedApprovalEntry()
        {
            TenantId = tenantId
        };
        return OperationResult<PostedApprovalEntry, DomainError>.Ok(entity);
    }
}
