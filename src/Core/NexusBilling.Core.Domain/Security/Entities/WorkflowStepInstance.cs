...
public class WorkflowStepInstance : Entity
{
    ...
    public static OperationResult<WorkflowStepInstance, DomainError> Create(TenantIdentifier tenantId)
    {
        if (tenantId == null || tenantId.Value == Guid.Empty)
            return OperationResult<WorkflowStepInstance, DomainError>.Fail(DomainError.Validation("security.tenant_required", "El TenantIdentifier es obligatorio."));

        var entity = new WorkflowStepInstance()
        {
            TenantId = tenantId
        };
        return OperationResult<WorkflowStepInstance, DomainError>.Ok(entity);
    }
}
